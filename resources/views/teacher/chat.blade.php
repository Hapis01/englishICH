...
<div class="flex flex-col md:grid md:grid-cols-4 gap-0 h-[80vh] md:h-[85vh] -mt-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="col-span-1 bg-white border-r border-gray-200 flex-col rounded-none {{ $selectedConversation ? 'hidden md:flex' : 'flex' }} h-full min-h-0">
        ...
        <div class="p-4 border-b border-gray-200 bg-gradient-to-r from-[#0B4637] to-[#10B981]">
            ...
            <p class="text-emerald-100 text-sm">Select student to chat</p>
        </div>

        <!-- Search -->
        <div class="p-4 border-b border-gray-200">
            <input type="text" id="conversationSearch" placeholder="Search students..." 
        ...
        <div class="flex-1 overflow-y-auto min-h-0" id="conversationsList">
            @forelse($chatStudents as $item)
                ...
                <div class="conversation-item p-3 border-b border-gray-100 hover:bg-emerald-50 cursor-pointer transition {{ $selectedConversation && $conversation && $selectedConversation->id == $conversation->id ? 'bg-emerald-100 border-l-4 border-[#10B981]' : '' }}"
                     onclick="{{ $conversation ? 'viewConversation(' . $conversation->id . ')' : 'startStudentConversation(' . $student->id . ')' }}">
        ...
        <!-- Pagination -->
        @if ($chatStudents->hasPages())
        <div class="p-3 border-t border-gray-200 flex justify-between items-center bg-gray-50">
            @if ($chatStudents->onFirstPage())
                <button disabled class="text-gray-400 px-3 py-1 text-sm rounded bg-gray-100 cursor-not-allowed">&laquo; Prev</button>
            @else
                <a href="{{ $chatStudents->previousPageUrl() }}" class="text-[#10B981] hover:bg-emerald-100 px-3 py-1 rounded text-sm font-medium transition bg-white border border-emerald-200">&laquo; Prev</a>
            @endif
            
            <span class="text-xs text-gray-500 font-medium">Page {{ $chatStudents->currentPage() }}</span>

            @if ($chatStudents->hasMorePages())
                <a href="{{ $chatStudents->nextPageUrl() }}" class="text-[#10B981] hover:bg-emerald-100 px-3 py-1 rounded text-sm font-medium transition bg-white border border-emerald-200">Next &raquo;</a>
            @else
        ...
        @if($selectedConversation)
            ...
            <div class="p-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <a href="{{ route('teacher.chat') }}" class="md:hidden p-2 -ml-2 text-gray-600 hover:bg-gray-200 rounded-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                    </a>
                    @if($selectedConversation->student->profile_photo)
                        <img src="{{ asset($selectedConversation->student->profile_photo) }}" 
                    ...
                    @endif
                    <div>
                        <h3 class="font-bold text-gray-900">{{ $selectedConversation->student->name }}</h3>
                        <p class="text-sm text-gray-600">{{ $selectedConversation->student->email }}</p>
                    </div>
                </div>
            </div>
        ...
        @else
            ...
            <div class="flex-1 flex items-center justify-center">
                <div class="text-center">
                    ...
                    <p class="text-gray-600 mb-6">Choose a student from the list or start a new conversation</p>
                    <button onclick="openAddStudentModal()" 
...
<div id="addStudentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 max-h-96 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-2" id="studentsList">
...
function loadAvailableStudents() {
    fetch('{{ route("teacher.chat.available-students") }}')
            ...
            if (data.success && data.data.length > 0) {
                studentsList.innerHTML = data.data.map(student => `
                    <div class="p-3 border border-gray-200 rounded-lg hover:bg-emerald-50 cursor-pointer transition"
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="w-10 h-10 rounded-full flex-shrink-0 ${student.profile_photo ? 'overflow-hidden' : 'bg-[#10B981] flex items-center justify-center text-white font-semibold text-sm'}">
                                ${student.profile_photo ? `<img src="/storage/${student.profile_photo}" alt="${student.name}" class="w-full h-full object-cover">` : student.name.charAt(0)}
                            </div>
                            <div class="flex-1">
...
function selectStudent(studentId) {
    fetch('{{ route("teacher.chat.start") }}', {
        method: 'POST',
        headers: {
...
function startStudentConversation(studentId) {
    selectStudent(studentId);
}

function viewConversation(conversationId) {
    window.location.href = `/teacher/chat/${conversationId}/view`;
}
...
function sendMessage(conversationId) {
    ...
    fetch(`/teacher/chat/${conversationId}/send`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        },
...
async function loadConversationMessages(conversationId) {
    try {
        const response = await fetch(`/teacher/chat/${conversationId}/messages`);
